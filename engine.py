...
class Engine(object):
    ...
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        input_var = torch.autograd.Variable(self.state['image'])
        adj_var = torch.autograd.Variable(self.state['adj'])
        target_var = torch.autograd.Variable(self.state['target'])
        embed = self.state['embed']
        ...
        # compute output
        self.state['output'] = model(input_var,adj_var,embed)
    ...
    def learning(self, model, criterion, train_dataset, val_dataset, optimizer=None):
        ...
        train_loader = torch.utils.data.DataLoader(train_dataset,
                                                   batch_size=self.state['batch_size'], shuffle=True, collate_fn=collate_fn,
                                                   num_workers=self.state['workers'])

        val_loader = torch.utils.data.DataLoader(val_dataset,
                                                 batch_size=self.state['batch_size'], shuffle=False, collate_fn=collate_fn,
                                                 num_workers=self.state['workers'])
        ...
        for epoch in range(self.state['start_epoch'], self.state['max_epochs']):
            ...
            # print(prec1,self.state['best_score'])
    ...
    def train(self, data_loader, model, criterion, optimizer, epoch):
        ...
        for i, (input, adj, target, len_target) in enumerate(data_loader):
            # measure data loading time
            self.state['iteration'] = i
            self.state['data_time_batch'] = time.time() - end
            self.state['data_time'].add(self.state['data_time_batch'])

            # print('[engine input]:',input)
            self.state['input'] = input
            self.state['adj'] = adj
            self.state['target'] = target
            self.state['cap_len'] = len_target
    ...
    def validate(self, data_loader, model, criterion):
        ...
        for i, (input, adj, target, len_target) in enumerate(data_loader):
            ...
            # print('[engine/validation input]:', input[2].shape)
            self.state['input'] = input
            self.state['adj'] = adj
            self.state['target'] = target
            self.state['cap_len'] = len_target
...
class MultiLabelMAPEngine(Engine):
    ...
    def on_end_epoch_old(self, training, model, criterion, data_loader, optimizer=None, display=True):
        # print('[engine/on_end_epoch difficult_examples]:',self.state['difficult_examples'])
    ...
    def on_end_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        # measure mAP
        # print('[engine/on_end_batch state-output]:',self.state['output'].data)
...
class GCNMultiLabelMAPEngine(MultiLabelMAPEngine):
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        if not training:
            with torch.no_grad():
                feature_var = torch.autograd.Variable(self.state['input']).float()
                adj_var = torch.autograd.Variable(self.state['adj']).float().detach()  # one hot
                target_var = torch.autograd.Variable(self.state['target'])
                target_len_var = self.state['cap_len']
        else:
            feature_var = torch.autograd.Variable(self.state['input']).float()
            adj_var = torch.autograd.Variable(self.state['adj']).float().detach()  # one hot
            target_var = torch.autograd.Variable(self.state['target'])
            target_len_var = self.state['cap_len']
        # compute output
        self.state['output'] = model(feature_var, adj_var,target_var,target_len_var)
        self.state['loss'] = criterion(self.state['output'], self.state['target_gt'].cuda())
    ...
    def on_start_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        self.state['target_gt'] = pack_padded_sequence(self.state['target'], self.state['cap_len'], batch_first=True).data
